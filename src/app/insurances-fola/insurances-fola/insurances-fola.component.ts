...
@Component({
  selector: 'app-insurances-fola',
  templateUrl: './insurances-fola.component.html',
  styleUrl: './insurances-fola.component.sass'
})
export class InsurancesFolaComponent {
    ...
    ngOnInit(): void {
        this.navigationService.setTitle('Fola');
        ...
        this.handleSearch$ = this.navigationService.handleSearch().subscribe(key => {
            ...
            this.insurancesFolaService.getInsurancesFolaByKey(key).subscribe(insurances => {
        ...
        this.handleClickMenu$ = this.navigationService.handleClickMenu().subscribe(id => {
            ...
            if (id == 'export_excel') {
                ...
                for (let index = 0; index < this.length / chunk; index++) {
                    const promise = lastValueFrom(this.insurancesFolaService.getInsurancesFolaByPage(index + 1, chunk, params))
                ...
                Promise.all(promises).then(values => {
                    ...
                    const name = `SEGUROS_Fola_${formatDate(new Date(), 'dd/MM/yyyy', 'en-US')}`;
    ...
    async onDelete(insuranceId: string) {
        ...
        if (ok) {
            this.insurancesFolaService.delete(insuranceId).subscribe(() => {
    ...
    fetchData() {
        ...
        this.insurancesFolaService.getCountInsurancesFola(params).subscribe(count => {
            this.length = count;
        });

        this.navigationService.loadBarStart();
        this.insurancesFolaService.getInsurancesFolaByPage(this.pageIndex + 1, this.pageSize, params).subscribe(insurancesFola => {
            this.navigationService.loadBarFinish();
            this.dataSource = insurancesFola;
        });
    }
