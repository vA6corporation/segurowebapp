...
@Component({
  selector: 'app-insurances-rcivil',
  templateUrl: './insurances-rcivil.component.html',
  styleUrl: './insurances-rcivil.component.sass'
})
export class InsurancesRcivilComponent {

    constructor(
        private readonly navigationService: NavigationService,
        private readonly insurancesRcivilService: InsurancesRcivilService,
    ...
    ngOnInit(): void {
        this.navigationService.setTitle('Rcivil');
        ...
        this.handleSearch$ = this.navigationService.handleSearch().subscribe(key => {
            ...
            this.insurancesRcivilService.getInsurancesRcivilByKey(key).subscribe(insurances => {
        ...
        this.handleClickMenu$ = this.navigationService.handleClickMenu().subscribe(id => {
            ...
            if (id == 'export_excel') {
                ...
                for (let index = 0; index < this.length / chunk; index++) {
                    const promise = lastValueFrom(this.insurancesRcivilService.getInsurancesRcivilByPage(index + 1, chunk, params))
                ...
                Promise.all(promises).then(values => {
                    ...
                    const name = `SEGUROS_Rcivil_${formatDate(new Date(), 'dd/MM/yyyy', 'en-US')}`;
    ...
    async onDelete(insuranceId: string) {
        ...
        if (ok) {
            this.insurancesRcivilService.delete(insuranceId).subscribe(() => {
    ...
    fetchData() {
        ...
        this.insurancesRcivilService.getCountInsurancesRcivil(params).subscribe(count => {
            this.length = count;
        });

        this.navigationService.loadBarStart();
        this.insurancesRcivilService.getInsurancesRcivilByPage(this.pageIndex + 1, this.pageSize, params).subscribe(insurancesRcivil => {
            this.navigationService.loadBarFinish();
            this.dataSource = insurancesRcivil;
        });
    }
