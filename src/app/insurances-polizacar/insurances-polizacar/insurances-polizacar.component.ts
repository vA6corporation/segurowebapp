...
@Component({
  selector: 'app-insurances-polizacar',
  templateUrl: './insurances-polizacar.component.html',
  styleUrl: './insurances-polizacar.component.sass'
})
export class InsurancesPolizacarComponent {
    ...
    ngOnInit(): void {
        this.navigationService.setTitle('Polizacar');
        ...
        this.handleSearch$ = this.navigationService.handleSearch().subscribe(key => {
            ...
            this.insurancesPolizacarService.getInsurancesPolizacarByKey(key).subscribe(insurances => {
        ...
        this.handleClickMenu$ = this.navigationService.handleClickMenu().subscribe(id => {
            ...
            if (id == 'export_excel') {
                ...
                for (let index = 0; index < this.length / chunk; index++) {
                    const promise = lastValueFrom(this.insurancesPolizacarService.getInsurancesPolizacarByPage(index + 1, chunk, params))
                ...
                Promise.all(promises).then(values => {
                    ...
                    const name = `SEGUROS_Polizacar_${formatDate(new Date(), 'dd/MM/yyyy', 'en-US')}`;
    ...
    async onDelete(insuranceId: string) {
        ...
        if (ok) {
            this.insurancesPolizacarService.delete(insuranceId).subscribe(() => {
    ...
    fetchData() {
        ...
        this.insurancesPolizacarService.getCountInsurancesPolizacar(params).subscribe(count => {
            this.length = count;
        });

        this.navigationService.loadBarStart();
        this.insurancesPolizacarService.getInsurancesPolizacarByPage(this.pageIndex + 1, this.pageSize, params).subscribe(insurancesPolizacar => {
            this.navigationService.loadBarFinish();
            this.dataSource = insurancesPolizacar;
        });
    }
