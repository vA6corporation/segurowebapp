...
@Component({
  selector: 'app-insurances-eps',
  templateUrl: './insurances-eps.component.html',
  styleUrl: './insurances-eps.component.sass'
})
export class InsurancesEpsComponent {

    constructor(
        private readonly navigationService: NavigationService,
        private readonly insurancesEpsService: InsurancesEpsService,
    ...
    ngOnInit(): void {
        this.navigationService.setTitle('Eps');
        ...
        this.handleSearch$ = this.navigationService.handleSearch().subscribe(key => {
            ...
            this.insurancesEpsService.getInsurancesEpsByKey(key).subscribe(insurances => {
        ...
        this.handleClickMenu$ = this.navigationService.handleClickMenu().subscribe(id => {
            ...
            if (id == 'export_excel') {
                ...
                for (let index = 0; index < this.length / chunk; index++) {
                    const promise = lastValueFrom(this.insurancesEpsService.getInsurancesEpsByPage(index + 1, chunk, params))
                ...
                Promise.all(promises).then(values => {
                    ...
                    const name = `SEGUROS_Eps_${formatDate(new Date(), 'dd/MM/yyyy', 'en-US')}`;
    ...
    async onDelete(insuranceId: string) {
        ...
        if (ok) {
            this.insurancesEpsService.delete(insuranceId).subscribe(() => {
    ...
    fetchData() {
        ...
        this.insurancesEpsService.getCountInsurancesEps(params).subscribe(count => {
            this.length = count;
        });

        this.navigationService.loadBarStart();
        this.insurancesEpsService.getInsurancesEpsByPage(this.pageIndex + 1, this.pageSize, params).subscribe(insurancesEps => {
            this.navigationService.loadBarFinish();
            this.dataSource = insurancesEps;
        });
    }
