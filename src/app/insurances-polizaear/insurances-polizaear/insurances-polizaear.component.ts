...
@Component({
  selector: 'app-insurances-polizaear',
  templateUrl: './insurances-polizaear.component.html',
  styleUrl: './insurances-polizaear.component.sass'
})
export class InsurancesPolizaearComponent {
    ...
    ngOnInit(): void {
        this.navigationService.setTitle('Polizaear');
        ...
        this.handleSearch$ = this.navigationService.handleSearch().subscribe(key => {
            ...
            this.insurancesPolizaearService.getInsurancesPolizaearByKey(key).subscribe(insurances => {
        ...
        this.handleClickMenu$ = this.navigationService.handleClickMenu().subscribe(id => {
            ...
            if (id == 'export_excel') {
                ...
                for (let index = 0; index < this.length / chunk; index++) {
                    const promise = lastValueFrom(this.insurancesPolizaearService.getInsurancesPolizaearByPage(index + 1, chunk, params))
                ...
                Promise.all(promises).then(values => {
                    ...
                    const name = `SEGUROS_Polizaear_${formatDate(new Date(), 'dd/MM/yyyy', 'en-US')}`;
    ...
    async onDelete(insuranceId: string) {
        ...
        if (ok) {
            this.insurancesPolizaearService.delete(insuranceId).subscribe(() => {
    ...
    fetchData() {
        ...
        this.insurancesPolizaearService.getCountInsurancesPolizaear(params).subscribe(count => {
            this.length = count;
        });
