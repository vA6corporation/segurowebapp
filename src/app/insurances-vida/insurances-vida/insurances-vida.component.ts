...
@Component({
  selector: 'app-insurances-vida',
  templateUrl: './insurances-vida.component.html',
  styleUrl: './insurances-vida.component.sass'
})
export class InsurancesVidaComponent {
    ...
    ngOnInit(): void {
        this.navigationService.setTitle('Vida');
        ...
        this.handleSearch$ = this.navigationService.handleSearch().subscribe(key => {
            ...
            this.insurancesVidaService.getInsurancesVidaByKey(key).subscribe(insurances => {
        ...
        this.handleClickMenu$ = this.navigationService.handleClickMenu().subscribe(id => {
            ...
            if (id == 'export_excel') {
                ...
                for (let index = 0; index < this.length / chunk; index++) {
                    const promise = lastValueFrom(this.insurancesVidaService.getInsurancesVidaByPage(index + 1, chunk, params))
                ...
                Promise.all(promises).then(values => {
                    ...
                    const name = `SEGUROS_Vida_${formatDate(new Date(), 'dd/MM/yyyy', 'en-US')}`;
    ...
    async onDelete(insuranceId: string) {
        ...
        if (ok) {
            this.insurancesVidaService.delete(insuranceId).subscribe(() => {
    ...
    fetchData() {
        ...
        this.insurancesVidaService.getCountInsurancesVida(params).subscribe(count => {
            this.length = count;
        });

        this.navigationService.loadBarStart();
        this.insurancesVidaService.getInsurancesVidaByPage(this.pageIndex + 1, this.pageSize, params).subscribe(insurancesVida => {
            this.navigationService.loadBarFinish();
            this.dataSource = insurancesVida;
        });
    }
