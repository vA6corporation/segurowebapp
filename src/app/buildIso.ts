...
import { IsoModel } from "./isos/iso.model";
...
export async function buildIso(
  ...
  const header = 11;
  const body = 8.5;
  const { business, company, partnership } = iso;
  ...
  text = `Fecha: ${formatDate(new Date(iso.createdAt), 'dd/MM/yyyy', 'en-US')}`;
  ...
  autoTable(pdf, {
    ...
    body: [
      [ { content: `SERVICIO DE ASESORIA Y GESTION DE LINEA DE CREDITO ${partnership ? 'CONSORCIO ' + partnership.name : ''}`, colSpan: 1 }, { content: `S/ ${(iso.commission || 0).toLocaleString('en-US', { minimumFractionDigits: 2 })}`, styles: { halign: 'right', cellWidth: 30 } } ],
      [],
      [],
      [],
    ],
