...
@Component({
  selector: 'app-insurances-pempresarial',
  templateUrl: './insurances-pempresarial.component.html',
  styleUrl: './insurances-pempresarial.component.sass'
})
export class InsurancesPempresarialComponent {
    ...
    ngOnInit(): void {
        this.navigationService.setTitle('Pempresarial');
        ...
        this.handleSearch$ = this.navigationService.handleSearch().subscribe(key => {
            ...
            this.insurancesPempresarialService.getInsurancesPempresarialByKey(key).subscribe(insurances => {
        ...
        this.handleClickMenu$ = this.navigationService.handleClickMenu().subscribe(id => {
            ...
            if (id == 'export_excel') {
                ...
                for (let index = 0; index < this.length / chunk; index++) {
                    const promise = lastValueFrom(this.insurancesPempresarialService.getInsurancesPempresarialByPage(index + 1, chunk, params))
                ...
                Promise.all(promises).then(values => {
                    ...
                    const name = `SEGUROS_Pempresarial_${formatDate(new Date(), 'dd/MM/yyyy', 'en-US')}`;
    ...
    async onDelete(insuranceId: string) {
        ...
        if (ok) {
            this.insurancesPempresarialService.delete(insuranceId).subscribe(() => {
    ...
    fetchData() {
        ...
        this.insurancesPempresarialService.getCountInsurancesPempresarial(params).subscribe(count => {
            this.length = count;
        });

        this.navigationService.loadBarStart();
        this.insurancesPempresarialService.getInsurancesPempresarialByPage(this.pageIndex + 1, this.pageSize, params).subscribe(insurancesPempresarial => {
            this.navigationService.loadBarFinish();
            this.dataSource = insurancesPempresarial;
        });
    }
